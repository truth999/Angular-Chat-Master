...
@Component({
  ...
  styleUrls: ['./dashboard-channel.component.scss']
})
export class DashboardChannelComponent implements OnInit {
  ...
  // @ViewChild('channelButton') channelButton: ElementRef;
  ...
  constructor( private renderer: Renderer2, private store: Store<AppState>, public chatService: ChatService, public emailService: EmailService, public voiceService: VoiceService) {
    this.renderer.listen('window', 'click',(e:Event)=>{
        // if(e.target == this.channelButton.nativeElement ){
        // }
        if(this.hover == false)  
        {
          this.isShow = false;
  ...
  ngOnInit(): void {
    ...
    // alert(this.channel[this.type].agentStatus.countUp);
  }
